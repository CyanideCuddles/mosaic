document.addEventListener("DOMContentLoaded", () => {
    ...
    // 11. Registration form — submits to Web3Forms via fetch so the visitor
    // stays on this page and sees a styled confirmation instead of being
    // redirected. Compiles all fields into one email to the inbox tied to
    // the access key below.
    ...
    if (registerForm) {
        ...
        registerForm.addEventListener('submit', async (event) => {
            ...
            const accessKey = registerForm.querySelector('input[name="access_key"]').value;

            if (!accessKey || accessKey.startsWith('PASTE_')) {
                statusEl.textContent = 'Form isn\'t connected yet — add a Web3Forms access key in register.html.';
